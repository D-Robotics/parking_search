...
#ifndef PARKING_SEARCH_ENGINE_H
#define PARKING_SEARCH_ENGINE_H
...
class ParkingSearchEngine {
 public:
  static std::shared_ptr<ParkingSearchEngine> Instance();
  ~ParkingSearchEngine();
  ...
  std::vector<std::shared_ptr<rclcpp::Node>> GetNodes() {
    ...
    node_ptrs.push_back(parking_search_node_);
    return node_ptrs;
  }
 ...
 private:
  ...
  std::shared_ptr<ParkingSearchNode> parking_search_node_ = nullptr;
